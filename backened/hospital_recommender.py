...
class HospitalRecommender:
    """
    Recommends hospitals with DISTANCE as primary factor:
    1. DISTANCE (60-80% weight) - Nearest hospitals prioritized
    2. Specialty match (10-20%)
    3. Hospital ratings (10%)
    4. Emergency availability (10%)
    """
    ...
    def find_nearby_hospitals(self, user_lat: float, user_lon: float,
        ...
        """
        Find and rank nearby hospitals - NEAREST FIRST
        
        Primary sort: DISTANCE
        Secondary sort: Specialty match + rating
        """
        ...
        # CRITICAL: Sort by distance FIRST, then by total score
        # This ensures nearest hospitals always appear first
        scored_hospitals.sort(key=lambda x: (x['distance_km'], -x['total_score']))
        
        # For non-critical cases, we can use a hybrid approach:
        # Group by distance tiers, then sort by score within each tier
        if priority != 'critical':
            # Tier 1: 0-5km (nearest)
            # Tier 2: 5-15km (nearby)
            # Tier 3: 15km+ (far)
            
            tier1 = [h for h in scored_hospitals if h['distance_km'] <= 5]
            tier2 = [h for h in scored_hospitals if 5 < h['distance_km'] <= 15]
            tier3 = [h for h in scored_hospitals if h['distance_km'] > 15]
            
            # Sort each tier by score
            tier1.sort(key=lambda x: -x['total_score'])
            tier2.sort(key=lambda x: -x['total_score'])
            tier3.sort(key=lambda x: -x['total_score'])
            
            # Combine tiers
            scored_hospitals = tier1 + tier2 + tier3
